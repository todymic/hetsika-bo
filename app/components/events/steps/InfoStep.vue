...
// Sync uploadedFiles → store.info.files
watch(uploadedFiles, (files) => {
  store.info.files        = files.filter(f => !f.error).map(f => f.file)
  store.info.uploadedFiles = files
}, { deep: true })

// Sync existingFiles → store
watch(existingFiles, (files) => {
  store.info.existingFiles = files
}, { deep: true })

// Sync removedFileIds → store
watch(removedFileIds, (ids) => {
  store.info.removedFileIds = ids
}, { deep: true })
...
onMounted(async () => {
  ...
  // Init existing files from store (mode edit)
  if (store.info.existingFiles?.length) {
    setExistingFiles(store.info.existingFiles)
  }
})
...
async function validate(): Promise<boolean> {
  ...
  if (!validateFiles(store.isEditMode)) {
    fileError.value = uploadError.value
    return false
  }
  fileError.value = ''
    ...
    <UForm ref="form" :schema="schema" :state="store.info" class="space-y-6">
      ...
      >
        ...
        <Transition enter-active-class="transition duration-200" enter-from-class="opacity-0">
          <p v-if="fileError || uploadError" class="mt-2 flex items-center gap-1.5 text-xs text-error">
            ...
            >
              <div class="h-10 w-10 shrink-0 overflow-hidden rounded-md border border-default bg-muted/40">
                ...
                <div v-else class="flex h-full w-full items-center justify-center">
                  <UIcon
                    :name="f.error ? 'i-lucide-alert-circle' : 'i-lucide-image'"
                    class="size-4"
